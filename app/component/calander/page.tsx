...
const AppMaster: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedNotes = localStorage.getItem('calendar_app_notes_v1');
  ...
  useEffect(() => { if (isMounted) localStorage.setItem('calendar_app_notes_v1', JSON.stringify(notes)); }, [notes, isMounted]);
  ...
  // --- RANGE HELPERS ---
  const dateToStr = (d: Date) => `${d.getFullYear()}-${(d.getMonth() + 1).toString().padStart(2, '0')}-${d.getDate().toString().padStart(2, '0')}`;
  const rangeStartStr = rangeStart ? dateToStr(rangeStart) : '';
  const rangeEndStr = rangeEnd ? dateToStr(rangeEnd) : '';
  const activeNoteKey = (isRangeMode && rangeStart && rangeEnd)
    ? `range:${rangeStartStr}:${rangeEndStr}`
    : selectedDateStr;
  const currentNote = notes[activeNoteKey] || '';
    ...
    <>
      ...
      <div className={`h-[100dvh] w-full relative overflow-hidden font-sans text-stone-800 flex justify-center selection:bg-black/10 transition-colors duration-700 bg-[#F4F2EC]`}>
        ...
        <div className="absolute top-[170px] left-0 w-full flex justify-center px-4 pointer-events-none z-10">
          <div className={`w-full max-w-md sm:max-w-xl md:max-w-2xl rounded-[32px] bg-cover bg-center shadow-sm transition-all duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${isExpanded ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`} style={{ backgroundImage: `url("${activeImage}")`, height: 'calc(100dvh - 300px)' }}>
        ...
        <div className="absolute left-0 w-full flex justify-center px-4 pointer-events-none transition-all duration-[800ms] ease-[cubic-bezier(0.22,1,0.36,1)] z-30" style={{ top: isExpanded ? '170px' : `calc(100dvh - ${cardHeight + 20}px)` }}>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-2 pointer-events-auto">
                  <button
                    ...
                    style={{ backgroundColor: isRangeMode ? monthTheme[2].bg : monthTheme[0].bg, color: isRangeMode ? monthTheme[2].text : monthTheme[2].text }}
                  >
                  ...
                  {calendarGrid.map((row, rowIndex) => (
                    <React.Fragment key={rowIndex}>
                      {row.map((col, colIndex) => {
                        const cellDateStr = col.dayNum ? `${currentMonthDate.getFullYear()}-${(currentMonthDate.getMonth() + 1).toString().padStart(2, '0')}-${col.dayNum.toString().padStart(2, '0')}` : '';
        ...
        <div
          ...
          style={{
            top: isExpanded ? `${170 + cardHeight + 10}px` : '100dvh', height: 'calc(100dvh - 170px)',
        ...
        >
          <div className="w-full max-w-md sm:max-w-xl md:max-w-2xl flex-1 bg-white/95 backdrop-blur-xl rounded-t-[40px] shadow-[0_-15px_40px_-15px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden border-t border-white/60 pointer-events-auto transition-colors duration-700" style={{ backgroundColor: activeTabId === 'month' ? monthTheme[0].bg : dayTheme[0].bg }}>
